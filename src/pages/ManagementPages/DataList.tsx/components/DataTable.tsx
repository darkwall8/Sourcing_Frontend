        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-gray-200">
              {headers.map(header => (
                <th key={header.key} className="py-2 font-semibold px-2 text-sm text-gray-400">
                  {header.label}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {filteredData.map((row, rowIndex) => (
              <tr key={rowIndex} onClick={() => onClickRow(row.id)} className="border-b border-gray-200 hover:bg-primary/5 cursor-pointer">
                {headers.map(col => (
                  <td key={col.key} className="py-4 px-2">
                    {col.type === "boolean" ? (
                      <span
                        className={`text-sm px-2 w-16 inline-flex items-center justify-center py-1 rounded-md font-semibold border-2 ${
                          row[col.key]
