...
function CompanyRegistrationStep2( { handleSubmit } : { handleSubmit: (stepIndex: "next" | "prev") => void } ) {
    ...
    const isValid = () => {
        return !!companyAdresse.trim() && !!companyCorporate.trim() && isValidPassword(companyPassword);
      };
        ...
        <div className="flex justify-between h-full gap-4">
            <form className="flex flex-col justify-between h-full text-xs w-full">
                <div className="flex flex-col gap-4">
                    ...
                    <InputText label={t("company_registration.your_company_website")} placeholder={t("company_registration.company_website_placeholder")} isRequired={false} value={companyWebSite} handleChange={setCompanyWebSite} showValidationErrors={isValidatedValues} />
